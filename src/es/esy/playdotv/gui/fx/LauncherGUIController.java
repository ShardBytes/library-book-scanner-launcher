...
public class LauncherGUIController implements Initializable{
	...
	private void launchLBS(ActionEvent e){
		...
		ProcessBuilder pb = new ProcessBuilder(jvm_location, "-splash:resources/splash.png", "-jar", "resources/" + prefList.getItems().get((selection == -1 ? 0 : selection)) + ".jar");
	...
	private void doUpdate(ActionEvent e) throws IOException{
		if(isLBSRunning){
			/*
			 * TODO: Fix freeze when LBS instance is already running
			 */
			Platform.runLater(() -> JOptionPane.showMessageDialog(null, "Ukon\u010Dite LBS a sk\u00FAste znova.", "Chyba", JOptionPane.ERROR_MESSAGE));
			return;
		}
		...
		Path out = Paths.get("resources/" + AutoUpdate.GithubData.getRelease_name() + ".jar");
	...
	private void addFilesToList(){
		File[] versions = new File("resources/").listFiles();
		
		for(File f : versions){
			if(f.isDirectory()){
				continue;
			}else{
				if(f.toString().startsWith("resources/Library Book Scanner [v") && f.toString().endsWith("].jar")){
